...
class Preprocessor(object):
    ...
    def read_data(self, filepath):
        ...
        with open(filepath, "r", encoding="utf8") as input_data:
            ...
            for line in input_data:
                ...
                hypotheses.append(
                    [w for w in hypothesis.rstrip().split() if w not in self.stopwords]
                )
    ...
    def read_data_xnli(self, filepath):
        """
        Read the premises, hypotheses and labels from some NLI dataset's
        file and return them in a dictionary. The file should be in the same
        form as SNLI's .txt files.
        ...
        """
        with open(filepath, "r", encoding="utf8") as input_data:
            langs, ids, premises, hypotheses, labels = defaultdict(dict), [], [], [], []
            ...
            for line in input_data:
                ...
                langs[line[0]]["premises"].append(
                    [w for w in premise.rstrip().split() if w not in self.stopwords]
                )
                langs[line[0]]["hypotheses"].append(
                    [w for w in hypothesis.rstrip().split() if w not in self.stopwords]
                )
                langs[line[0]]["labels"].append(line[1])
                langs[line[0]]["ids"].append(pair_id)
    ...
    def build_worddict(self, data):
        ...
        if self.labeldict == {}:
            ...
            self.labeldict = {label_name: i for i, label_name in enumerate(label_names)}

    def build_worddict_xnli(self, lang_data):
        """
        Build a dictionary associating words to unique integer indices for
        some dataset. The worddict can then be used to transform the words
        in datasets to their indices.
        ...
        for lang, data in lang_data.items():
            ...
            # Special indices are used for padding, out-of-vocabulary words, and
            # beginning and end of sentence tokens.
            self.worddict[lang]["_PAD_"] = 0
            self.worddict[lang]["_OOV_"] = 1

            offset = 2
            if self.bos:
                self.worddict[lang]["_BOS_"] = 2
                offset += 1
            if self.eos:
                self.worddict[lang]["_EOS_"] = 3
                offset += 1

            for i, word in enumerate(counts.most_common(num_words)):
                self.worddict[lang][word[0]] = i + offset

            if self.labeldict == {}:
                label_names = set(data["labels"])
                self.labeldict = {
                    label_name: i for i, label_name in enumerate(label_names)
                }
    ...
    def words_to_indices_xnli(self, sentence, lang):
        """
        Transform the words in a sentence to their corresponding integer
        indices.
        ...
        # if one is defined.
        if self.bos:
            indices.append(self.worddict[lang]["_BOS_"])

        for word in sentence:
            if word in self.worddict[lang]:
                index = self.worddict[lang][word]
            else:
                # Words absent from 'worddict[lang]' are treated as a special
                # out-of-vocabulary word (OOV).
                index = self.worddict[lang]["_OOV_"]
            indices.append(index)
        # Add the end of sentence token at the end of the sentence if one
        # is defined.
        if self.eos:
            indices.append(self.worddict[lang]["_EOS_"])
    ...
    def indices_to_words_xnli(self, indices, lang):
        """
        Transform the indices in a list to their corresponding words in
        the object's worddict.
        ...
        """
        return [
            list(self.worddict[lang].keys())[
                list(self.worddict[lang].values()).index(i)
            ]
            for i in indices
        ]

    def transform_to_indices_xnli(self, data):
        """
        ...
        for lang, lang_data in data.items():
            ...
            for i, premise in enumerate(lang_data["premises"]):
                ...
                indices = self.words_to_indices_xnli(premise, lang)
                transformed_data["premises"].append(indices)

                indices = self.words_to_indices_xnli(lang_data["hypotheses"][i], lang)
                transformed_data["hypotheses"].append(indices)
            langs_transformed[lang] = transformed_data

        return langs_transformed
    ...
    def transform_to_indices(self, data):
        ...
        Returns:
            ...
            labels.
        """
        transformed_data = {"ids": [], "premises": [], "hypotheses": [], "labels": []}
    ...
    def build_embedding_matrix_xnli(self, embeddings_file, lang):
        """
        Build an embedding matrix with pretrained weights for object's
        worddict.
        ...
        num_words = len(self.worddict[lang])
        embedding_dim = len(list(embeddings.values())[0])
        embedding_matrix = np.zeros((num_words, embedding_dim))

        # Actual building of the embedding matrix.
        missed = 0
        for word, i in self.worddict[lang].items():
            if word in embeddings:
                embedding_matrix[i] = np.array(embeddings[word], dtype=float)
            else:
                if word == "_PAD_":
...
class NLIDataset(Dataset):
    ...
    def __init__(
        self, data, padding_idx=0, max_premise_length=None, max_hypothesis_length=None
    ):
        """
        Args:
        ...
        self.data = {
            "ids": [],
            "premises": torch.ones(
                (self.num_sequences, self.max_premise_length), dtype=torch.long
            )
            * padding_idx,
            "hypotheses": torch.ones(
                (self.num_sequences, self.max_hypothesis_length), dtype=torch.long
            )
            * padding_idx,
    ...
    def __getitem__(self, index):
        return {
            ...
            "premise_length": min(
                self.premises_lengths[index], self.max_premise_length
            ),
            "hypothesis": self.data["hypotheses"][index],
            "hypothesis_length": min(
                self.hypotheses_lengths[index], self.max_hypothesis_length
            ),
