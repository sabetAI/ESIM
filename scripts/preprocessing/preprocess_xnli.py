"""
Preprocess the XNLI dataset and word embeddings to be used by the ESIM model.
"""
# Aurelien Coet, 2018.
...
def preprocess_XNLI_data(
    ...
    """
    Preprocess the data from the XNLI corpus so it can be used by the
    ESIM model.
    ...
    print("\t* Reading data...")
    data = preprocessor.read_data_xnli(os.path.join(inputdir, dev_file))
    print("\t* Transforming words in premises and hypotheses to indices...")
    preprocessor.build_worddict_xnli(data)
    transformed_data = preprocessor.transform_to_indices_xnli(data)
    ...
    print("\t* Reading data...")
    data = preprocessor.read_data_xnli(os.path.join(inputdir, test_file))
    ...
    embed_matrix = preprocessor.build_embedding_matrix_xnli(embeddings_file, "fr")
    with open(os.path.join(targetdir, "embeddings.pkl"), "wb") as pkl_file:
        pickle.dump(embed_matrix, pkl_file)


if __name__ == "__main__":
    default_config = "../../config/preprocessing/xnli_preprocessing.json"
